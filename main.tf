resource "google_container_cluster" "primary" {
  ...
  location                 = data.terraform_remote_state.common_vars.outputs.region
  remove_default_node_pool = true
  initial_node_count       = 1
  resource_labels = {
    environment = data.terraform_remote_state.common_vars.outputs.env
    owner = data.terraform_remote_state.common_vars.outputs.owner
  }
